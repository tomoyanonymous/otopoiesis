...
impl<'a> FadeInOut<'a> {
    ...
    fn make_handle(
        ...
        let (paint_rect, handle_pos) = if is_start {
            let handle_pos = egui::pos2(target_rect.left() + fade_width, top);
            let rect = egui::Rect::from_points(&[target_rect.left_bottom(), handle_pos]);
            (rect, handle_pos)
        } else {
            let right = target_rect.left()
                + self.origin_ui.range.getrange() as f32 * PIXELS_PER_SEC_DEFAULT;
            let right_bottom = egui::pos2(right, target_rect.bottom());
            let handle_pos = egui::pos2(right - fade_width, top);
            let rect = egui::Rect::from_points(&[handle_pos, right_bottom]);
            (rect, handle_pos)
        };
